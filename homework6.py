...
class adiabatic_drag_cannon(cannon):
    def next_state(self, current_state):
        ...
        v = sqrt(current_state.vx * current_state.vx + current_state.vy * current_state.vy)
        next_x = current_state.x + current_state.vx * self.dt
        next_vx = current_state.vx - b2m*((1-(j*current_state.y)/(T0))**l)* v * current_state.vx * self.dt
        next_y = current_state.y + current_state.vy * self.dt
        next_vy = current_state.vy - g * self.dt - b2m*((1-(j*current_state.y)/(T0))**l)* v * current_state.vy * self.dt
        #print next_x, next_y
        return flight_state(next_x, next_y, next_vx, next_vy, current_state.t + self.dt)


a = cannon(flight_state(0, 0, 500, 500, 0), _dt = 0.1)
a.shoot()
b = drag_cannon(flight_state(0, 0, 500, 500, 0), _dt = 0.1)
b.shoot()
c = adiabatic_drag_cannon(flight_state(0, 0, 500, 500, 0), _dt = 0.1)
c.shoot()
a.show_trajectory()
b.show_trajectory()
c.show_trajectory()
